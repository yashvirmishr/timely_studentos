...
BASE = os.environ.get("TIMELY_BASE_URL", "http://127.0.0.1:3012/")
errors = []
...
with sync_playwright() as p:
    ...
    page.locator(".mobile-nav").evaluate("el => Array.from(el.children).at(-1)?.querySelector('button')?.click()")
    page.wait_for_timeout(100)
    assert page.locator(".mobile-more-menu").count() == 1
    assert page.locator(".mobile-more-menu button").count() == 5
